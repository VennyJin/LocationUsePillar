...
void dataAdjustToGuard( Mat & Ximg, Mat &Yimg, Mat &Zimg,Mat & Dimg,Mat &result)
{
	...
	for(int y=0;y<rows;y++)
	{
		...
		for(int x=0;x<cols;x++)
		{
			if(dData[x]>100&&dData[x]<500&&zData[x]>1800&&zData[x]<3500)
			{	
...
bool fit3DPillar(Mat &pillarMask)
{
	//提取二维图像的联通域
	...
	//for( ; idx >= 0; idx = hierarchy[idx][0] )
    {
		...
		//需要判定哪根柱子是哪根柱子
		PillarCameralocation[0].x=cameraLocation[0];
		PillarCameralocation[0].y=cameraLocation[1];
		PillarCameralocation[0].z=cameraLocation[2];
...
bool fit3DGuard(Mat &guardMask)
{
	...
	for(;idx<min(1,contours_num);idx++)
	{
		...
		int guardFitBoxHeight=min(100,cenContoursY-minContoursY);
