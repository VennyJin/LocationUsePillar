...
void calRotationMatrixbyGuard()
{
	...
	float g_yaw=atan2(rotationMatrixCameraToGuard_pie.at<float>(2,0),sqrt(rotationMatrixCameraToGuard_pie.at<float>(2,1)*rotationMatrixCameraToGuard_pie.at<float>(2,1)+rotationMatrixCameraToGuard_pie.at<float>(2,2)*rotationMatrixCameraToGuard_pie.at<double>(2,2)));
...
void UpdateCameraCoordinate(Mat &xImage,Mat &yImage,Mat &zImage,Mat & r_Image)
{
	...
	gemm(tmpRotation_f,out_put,1,NULL,0,out_put,0);
...
{
  //Points*R_T
  gemm(guard_points,rotateMatrixCameraToGround,1,NULL,0,guard_points,GEMM_2_T);
...
{
	gemm(guardPoints,rotationMatrixCameraToGuard_pie,1,NULL,0,guardPoints,GEMM_2_T);
...
{
	gemm(rotationMatrixCameraToGuard_pie,rotateMatrixCameraToGround,1,NULL,0,rotateMatrixCameraToWorld,NULL);
...
{
    Mat pillarLocation=Mat ::zeros(3,1,CV_32FC1);
	pillarLocation.at<float>(0,0)=PillarCameralocation[pillarId].x;
	pillarLocation.at<float>(1,0)=PillarCameralocation[pillarId].y;
	pillarLocation.at<float>(2,0)=PillarCameralocation[pillarId].z;
	
	Mat pillarFinalLocation;
	gemm(rotateMatrixCameraToWorld,pillarLocation,1,NULL,0,pillarFinalLocation,NULL);
	
	PilarWorldlocation[pillarId].x=pillarFinalLocation.at<float>(0,0);
	PilarWorldlocation[pillarId].y=pillarFinalLocation.at<float>(1,0);
	PilarWorldlocation[pillarId].z=pillarFinalLocation.at<float>(2,0);

#ifdef  WRITEPARAMETERS
	cout<<"PillarId: "<<pillarId<<" X: "<<PilarWorldlocation[pillarId].x<<" Y: "<<PilarWorldlocation[pillarId].y<<" Z: "<<PillarCameralocation[pillarId].z<<endl;
	PillarWorldLocationFile<<"World "<<PilarWorldlocation[pillarId].x<<" "<<PilarWorldlocation[pillarId].y<<" "<<PilarWorldlocation[pillarId].z<<endl;
	PillarWorldLocationFile<<"Camera "<<PillarCameralocation[pillarId].x<<" "<<PillarCameralocation[pillarId].y<<" "<<PillarCameralocation[pillarId].z<<endl;
#endif
