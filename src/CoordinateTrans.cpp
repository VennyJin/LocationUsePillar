...
Mat calRotationMatrix(float Equation[4],int flag)
{
    //世界坐标平面的法向量定义，与安装角有关
    double PlaneWorldNormalVector[3]={0.0,0.0,0.0};
    if(flag == 0)
    {
        PlaneWorldNormalVector[2]= 1.0; //需要事先标定相机安装的角度
    }
    else if (flag == 1)
    {
        PlaneWorldNormalVector[3]=1.0; //需要事先标定相机安装的角度
    }

    ////计算旋转轴
    float PlaneCameraNormalVector[3];   //groundWorldNormalVector叉乘拟合地面的方程
    double rotationAxis[3];
    ...
    for(int i=0;i<3;i++)
    {
        PlaneCameraNormalVector[i]=Equation[i];
    }


    //计算旋转角
    double crossProduct=abs(PlaneWorldNormalVector[0]*PlaneCameraNormalVector[0]+PlaneWorldNormalVector[1]*PlaneCameraNormalVector[1]+PlaneWorldNormalVector[2]*PlaneCameraNormalVector[2]);
    double theta=acos(crossProduct);
    //cout<<"角度: "<<theta*180/pi<<endl;
    //旋转前向量是 相机看到的向量 PlaneCameraNormalVector
    //旋转后向量是 规定的向量 PlaneWorldNormalVector

    //计算旋转向量
    rotationAxis[0]=PlaneCameraNormalVector[1]*PlaneWorldNormalVector[2]-PlaneCameraNormalVector[2]*PlaneWorldNormalVector[1];
    rotationAxis[1]=PlaneCameraNormalVector[2]*PlaneWorldNormalVector[0]-PlaneCameraNormalVector[0]*PlaneWorldNormalVector[2];
    rotationAxis[2]=PlaneCameraNormalVector[0]*PlaneWorldNormalVector[1]-PlaneCameraNormalVector[1]*PlaneWorldNormalVector[0];
